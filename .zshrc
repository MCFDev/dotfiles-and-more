...
# change these based on your system configuration if needed, should work out of the box on debian based systems.
LAN="eth0"
WLAN="wlan0"
VPN="tun0"
...
IP1=$(ip -4 addr | grep -v 127.0.0.1 | grep -v secondary | grep $LAN | grep -Po "inet \K[\d.]+") # Get normal interface, may need to be changed
IP2=$(ip -4 addr | grep -v 127.0.0.1 | grep -v secondary | grep $VPN | grep -Po "inet \K[\d.]+") # Get VPN IP if connected
IP3=$(ip -4 addr | grep -v 127.0.0.1 | grep -v secondary | grep $WLAN | grep -Po "inet \K[\d.]+") # Get Wireless IP if connected
